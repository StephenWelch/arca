...
def foot_height_l2(env, asset_cfg: SceneEntityCfg, sensor_cfg: SceneEntityCfg, height: float) -> torch.Tensor:
    """Reward feet height."""
    asset = env.scene[asset_cfg.name]
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    contact_time = contact_sensor.data.current_contact_time[:, sensor_cfg.body_ids]
    ...
    reward = torch.square(height - foot_height)
    return torch.sum(reward * ~in_contact, dim=1)

def foot_height_exp(env, asset_cfg: SceneEntityCfg, sensor_cfg: SceneEntityCfg, height: float, std: float) -> torch.Tensor:
    """Reward feet height."""
    asset = env.scene[asset_cfg.name]
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    contact_time = contact_sensor.data.current_contact_time[:, sensor_cfg.body_ids]
    in_contact = contact_time > 0.0
    foot_height = asset.data.body_pos_w[:, asset_cfg.body_ids, 2]
    reward = torch.exp(-torch.square(height - foot_height) / std**2)
    return torch.sum(reward * ~in_contact, dim=1)